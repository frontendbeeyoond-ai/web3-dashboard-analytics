...
export default function PresaleAnalyticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await fetch("/api/analytics/presale", { cache: "no-store" });
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error(body.details || body.error || "Failed to fetch presale analytics");
        }
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h1 className="text-xl font-bold text-slate-900">presale_click</h1>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {isLoading && !data && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="mt-4 text-sm text-slate-500">Loading presale analytics…</p>
          </div>
        )}
