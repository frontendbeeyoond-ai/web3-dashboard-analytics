...
import { fetchTrustpilotDetail } from "@/lib/ga4Utils";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await fetchTrustpilotDetail(filters);
    return NextResponse.json({ source: "ga4", ...data });
  } catch (error) {
    console.error("Trustpilot analytics API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch trustpilot analytics", details: String(error) },
      { status: 500 }
    );
  }
}
