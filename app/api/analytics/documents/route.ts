...
import { fetchDocumentsDetail } from "@/lib/ga4Utils";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await fetchDocumentsDetail(filters);
    return NextResponse.json({ source: "ga4", ...data });
  } catch (error) {
    console.error("Documents analytics API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch documents analytics", details: String(error) },
      { status: 500 }
    );
  }
}
