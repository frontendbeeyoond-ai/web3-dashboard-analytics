  ...
  if (data.length === 0) {
    return (
      <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <h3 className="text-lg font-semibold text-slate-900">Events Overview</h3>
        <p className="mt-1 text-sm text-slate-500">Distribution of tracked events</p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-200">
            {data.map((row, index) => (
              <tr key={row.eventName} className="hover:bg-slate-50">
                ...
                <td className="whitespace-nowrap px-6 py-4 text-right">
                  <span className={`text-sm font-medium ${
                    row.conversionRate > 5 ? "text-green-600" : row.conversionRate > 1 ? "text-amber-600" : "text-slate-600"
                  }`}>
                    {row.conversionRate}%
                  </span>
                </td>
